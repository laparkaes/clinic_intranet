...
document.addEventListener("DOMContentLoaded", () => {
	...
	function set_stock(dom){
		if ($(dom).val() == 71){//service type
			...
			$("#ep_stock").val(0);
		}else $("#ep_stock").prop("readonly", false);
	}
	
	//option
	$("#form_add_option").submit(function(e) {
		e.preventDefault();
		ajax_form_warning(this, "commerce/product/add_option", "wm_option_add").done(function(res) {
			swal_redirection(res.type, res.msg, window.location.href);
		});
	});
	...
	$(".form_edit_option").submit(function(e) {
		e.preventDefault();
		ajax_form_warning(this, "commerce/product/edit_option", "wm_option_edit").done(function(res) {
			swal_redirection(res.type, res.msg, window.location.href);
		});
	});
	
	$(".op_delete").click(function() {
		ajax_simple_warning({id: $(this).val()}, "commerce/product/delete_option", "wm_option_delete").done(function(res) {
			swal_redirection(res.type, res.msg, window.location.href);
		});
	});
	...
	$("#form_add_image").submit(function(e) {
		...
		ajax_form(this, "commerce/product/add_image").done(function(res) {
			swal(res.type, res.msg);
			if (res.type == "success"){
	...
	$(".btn_set_img").click(function() {
		set_product_image(this);
	});
	...
	$("#form_save_provider").submit(function(e) {
		...
		ajax_form(this, "commerce/product/save_provider").done(function(res) {
			set_msg(res.msgs);
			swal_redirection(res.type, res.msg, window.location.href);
		});
	});
	
	$("#btn_clean_provider").click(function() {
	...
	$("#form_edit_product").submit(function(e) {
		...
		ajax_form(this, "commerce/product/edit_product").done(function(res) {
			set_msg(res.msgs);
			swal_redirection(res.type, res.msg, window.location.href);
		});
	});
	
	$("#ep_type").change(function() {
		set_stock(this);
	});
	
	$(".btn_delete").click(function() {
		ajax_simple_warning({id: $(this).val()}, "commerce/product/delete_product", "wm_product_delete").done(function(res) {
